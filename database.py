...
class BaseDeDatos:
    ...
    def obtener_mensajes(self, id_usuario, destinatario):
        ...
        FROM MISSATGES_U mu
        JOIN usuarisclase u ON mu.id_usuario_envia = u.id
        WHERE (mu.id_usuario_envia = %s AND mu.id_usuario_recibe = %s)
            OR (mu.id_usuario_envia = %s AND mu.id_usuario_recibe = %s)
        ORDER BY mu.date DESC LIMIT 15;
        """
    ...
    def cambiar_estado_U(self, id_mensaje, estado):
        sql = "UPDATE MISSATGES_U SET estat = %s WHERE id = %s"
